...
class LoaderView: UIView {
    ...
    private func setPath() {
        layer.path = UIBezierPath(ovalIn: bounds.insetBy(
            dx: layer.lineWidth / 2,
    ...
    private struct Pose {
        ...
        let length: CGFloat
        init(
            _ secondsSincePriorPose: CFTimeInterval,
            _ start: CGFloat,
            _ length: CGFloat
        ) {
    ...
    private func animate() {
        ...
        let originRotation: CGFloat = -.pi/2
        
        for pose in poses {
            time += pose.secondsSincePriorPose
            times.append(time / totalSeconds)
            start = pose.start
            rotations.append(originRotation + (start * 2 * .pi))
